...
function effectiveRpcUrl(globalOpts) {
...
function print(data, format = "text") {
    if (format === "json") {
...
diagnostics
    ...
    try {
        ...
        checks.push({ check: "docker-daemon", ok: false, detail: `Docker unavailable: ${error.message}` });
    }
    checks.push({
    ...
    try {
        ...
        checks.push({ check: "rpc-health", ok: false, detail: `${rpcUrl}: ${error.message}` });
    }
    for (const [name, url] of [
        ...
        try {
            ...
            checks.push({ check: name, ok: false, detail: `${url}: ${error.message}` });
        }
    }
    if ((globalOpts.output ?? "text") === "json") {
        print({ checks }, "json");
...
validator
    ...
    try {
        ...
        print(stats, (globalOpts.output ?? "text"));
    }
    catch (error) {
        console.error(chalk.red(error.message));
        process.exitCode = 1;
...
seal
    ...
    try {
        const sealPayload = (await client.seals.get(sealId));
        ...
        print(result, (globalOpts.output ?? "text"));
        if (!result.valid)
            process.exitCode = 2;
    }
    catch (error) {
...
wallet
    ...
    try {
        ...
        print(balances, (globalOpts.output ?? "text"));
    }
    catch (error) {
        console.error(chalk.red(error.message));
        process.exitCode = 1;
...
wallet
    ...
    if (opts.out) {
        ...
        print(manifest, (globalOpts.output ?? "text"));
    }
});
...
program
    ...
    try {
        ...
        print(data, (globalOpts.output ?? "text"));
    }
    catch (error) {
        console.error(chalk.red(error.message));
        process.exitCode = 1;
