...
function toVerificationOptions(opts) {
...
function printResult(result, asJson) {
    ...
    console.log(chalk.cyan("Consensus:"), result.metadata.consensusBps !== undefined
    ...
    for (const check of result.checks) {
        ...
        const labelColor = check.severity === "error" ? chalk.red : check.severity === "warning" ? chalk.yellow : chalk.gray;
...
async function verifySingleSeal(seal, opts) {
...
program
    ...
    try {
        ...
        console.error(chalk.red(`verify-file failed: ${error.message}`));
        process.exitCode = 1;
    }
});
program
    ...
    try {
        ...
        console.error(chalk.red(`verify-stdin failed: ${error.message}`));
        process.exitCode = 1;
    }
});
program
    ...
    try {
        ...
        console.error(chalk.red(`batch-verify failed: ${error.message}`));
        process.exitCode = 1;
    }
});
program
    ...
    try {
        const seal = readJsonFile(file);
        const expiresAtRaw = (seal.expiresAt ?? seal?.seal?.expiresAt);
        if (!expiresAtRaw) {
        ...
        console.error(chalk.red(`check-expiry failed: ${error.message}`));
        process.exitCode = 1;
    }
});
program
