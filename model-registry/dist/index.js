...
function printEntry(entry, json) {
    if (json) {
...
program
    ...
    .option("--db <path>", "Registry database file path")
    .option("--json", "Output JSON")
    .action((modelHash, options) => {
    try {
        const dbPath = resolveDbPath(options.db);
        const db = loadDb(dbPath);
        const idx = db.models.findIndex((m) => m.modelHash === modelHash);
        if (idx === -1)
            throw new Error(`Model not found: ${modelHash}`);
...
program
    ...
    try {
        ...
        if (options.owner)
            models = models.filter((m) => m.owner === options.owner);
        if (options.category)
            models = models.filter((m) => (m.category ?? "general") === options.category);
...
program
    ...
    .option("--db <path>", "Registry database file path")
    .option("--json", "Output JSON")
    .action(async (modelHash, options) => {
